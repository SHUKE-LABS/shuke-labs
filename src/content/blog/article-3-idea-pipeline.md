...
An accepted idea becomes a ~~delivery-ready issue~~ `raw-report` issue — a blind-authored problem statement a delivery agent still has to refine and ground before it can be built — tagged and carrying its full receipt trail. That's where the auditor stops: it does not write code, and it does not merge. From there the idea travels the same path every one of our own tickets does — a delivery agent plans and builds it, an independent reviewer checks it, and a green test suite gates the merge. ~~Shipping still passes through a human merge gate — the same gate every one of our own tickets passes through.~~ That merge is *autonomous*; no human signs off on it, and ours don't either. Acceptance queues an idea for consideration, not a promise to ship — whether it endures is decided by humans and agents together as the roadmap moves.

That was the deliberate boundary — though not the one you might expect. ~~It would have been a short step to let the auditor open a pull request too. We didn't take it, because the~~ The interesting question was never "can an agent decide what's worth building" — it's "how much can it decide *safely on its own*, and how do you stop an untrusted stranger from steering it." The front door faces the world with no keys; the auditor judges but can't be steered and fails toward "no"; ~~and the last word before anything ships is still a person's.~~ and nothing a submitter writes can talk its way into the pipeline or onto a merge. The line we didn't cross was never human-versus-machine — it's letting the outside world's words become instructions.
