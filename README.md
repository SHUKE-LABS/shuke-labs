...
- **Production** (`shukelabs.com`, the `prod` branch) is a gated promotion: dispatch the **"Release to production"** workflow (a chosen ref → `prod`, promoted by manual dispatch) after beta is verified — nothing ships to production automatically on a merge to `main`.
